  ...
  Widget build(BuildContext context) {
    ...
    return ListView(
      ...
      children: [
        ...
        const SizedBox(height: 16),
        if (groups.length > 1)
          Wrap(spacing: 8, runSpacing: 8, children: groups.map((g) => ChoiceChip(avatar: Transform.scale(scale: 1.45, child: Image.asset(asset(g), width: 52, height: 52, fit: BoxFit.contain, filterQuality: FilterQuality.high)), label: Text(label(g)), selected: group == g, onSelected: busy ? null : (_) => setState(() => group = g))).toList()),
        if (groups.length == 1)
