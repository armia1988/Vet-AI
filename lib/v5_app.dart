  ...
  Widget build(BuildContext context) {
    ...
    return ListView(
      ...
      children: [
        ...
        const SizedBox(height: 16),
        if (groups.length > 1)
          Wrap(spacing: 8, runSpacing: 8, children: groups.map((g) => ChoiceChip(avatar: SizedBox(width: 56, height: 42, child: Image.asset(asset(g), fit: BoxFit.contain)), label: Text(label(g)), selected: group == g, onSelected: busy ? null : (_) => setState(() => group = g))).toList()),
        if (groups.length == 1)
