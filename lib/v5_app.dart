...
class _V5AuthScreenState extends State<V5AuthScreen> {
  ...
  Future<void> submit() async {
    FocusScope.of(context).unfocus();
    if (email.text.trim().isEmpty || password.text.length < 6 || (create && fullName.text.trim().isEmpty)) {
      _snack(tr(
        context,
    ...
    try {
      ...
      if (mounted) {
        _snack(tr(context, 'Unable to complete account request.', 'تعذر إكمال طلب الحساب.', 'De accountaanvraag kon niet worden voltooid.'), true);
      }
    } finally {
          ...
          children: [
            ...
            if (confirmationEmail != null) ...[
              _Notice(
                ...
                title: tr(context, 'Confirm your email', 'أكد بريدك الإلكتروني', 'Bevestig je e-mailadres'),
                text: tr(
                  context,
            ...
            SegmentedButton<bool>(
              segments: [
                ButtonSegment(value: true, icon: const Icon(Icons.person_add_alt_1_rounded, size: 26), label: Text(tr(context, 'Create account', 'إنشاء حساب', 'Account aanmaken'))),
                ButtonSegment(value: false, icon: const Icon(Icons.login_rounded, size: 26), label: Text(tr(context, 'Sign in', 'تسجيل الدخول', 'Inloggen'))),
              ],
              selected: {create},
              onSelectionChanged: busy ? null : (v) => setState(() => create = v.first),
            ),
            const SizedBox(height: 22),
            if (create) ...[
              _Field(controller: fullName, label: tr(context, 'Full name', 'الاسم الكامل', 'Volledige naam'), icon: Icons.badge_outlined),
              const SizedBox(height: 12),
              _Field(controller: phone, label: tr(context, 'Phone', 'الهاتف', 'Telefoon'), icon: Icons.phone_outlined, keyboard: TextInputType.phone),
              const SizedBox(height: 12),
            ],
      ...
      body: Center(
        child: Padding(
          ...
          child: _Notice(
            ...
            title: tr(context, 'Email confirmation required', 'تأكيد البريد مطلوب', 'E-mailbevestiging vereist'),
            text: tr(
              context,
...
class _V5OnboardingScreenState extends State<V5OnboardingScreen> {
  ...
  @override
  void dispose() {
    for (final c in [company, farmName, country, region, workers, vets, barns, area, livestock, poultry, dogs, breeds, age, purpose, ventilation, vaccines, history]) {
      c.dispose();
    }
    super.dispose();
  }
  ...
  void _toggleGroup(String group) {
    setState(() {
      ...
      if (selectedGroups.contains('livestock') && _i(livestock) == 0) livestock.text = '1';
      if (selectedGroups.contains('poultry') && _i(poultry) == 0) poultry.text = '1';
      if (selectedGroups.contains('dogs') && _i(dogs) == 0) dogs.text = '1';
    });
  }

  Future<void> finish() async {
    if (farmName.text.trim().isEmpty || _i(barns, 1) < 1 || selectedGroups.isEmpty) return;
    setState(() => busy = true);
    try {
      await VetBackend.instance.createFarm(FarmSetupPayload(
        ...
        livestockCount: selectedGroups.contains('livestock') ? _i(livestock, 1) : 0,
      ...
      if (!mounted) return;
      Navigator.of(context).pushAndRemoveUntil(MaterialPageRoute(builder: (_) => const V5AuthGate()), (_) => false);
    } catch (_) {
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(SnackBar(
          content: Text(tr(context, 'The farm could not be created safely. Try again.', 'تعذر إنشاء المزرعة بأمان. حاول مرة أخرى.', 'De boerderij kon niet veilig worden aangemaakt. Probeer opnieuw.')),
    ...
    return Scaffold(
      appBar: AppBar(
        leading: IconButton(
          ...
          onPressed: busy ? null : () => step > 0 ? setState(() => step--) : VetBackend.instance.signOut(),
        ),
        title: Row(children: [const _MiniBrand(), const SizedBox(width: 12), Text(tr(context, 'Farm setup', 'إعداد المزرعة', 'Boerderij instellen'))]),
      ),
      body: Column(
        children: [
            ...
            child: ListView(
              ...
              children: [
                ...
                ElevatedButton.icon(
                  ...
                  icon: busy
                      ? const SizedBox.square(dimension: 22, child: CircularProgressIndicator(strokeWidth: 2))
                      : Icon(step < 2 ? Icons.arrow_forward_rounded : Icons.check_circle_outline_rounded, size: 29),
                  label: Text(step < 2 ? tr(context, 'Continue', 'متابعة', 'Doorgaan') : tr(context, 'Create farm', 'إنشاء المزرعة', 'Boerderij aanmaken')),
                ),
              ],
            ),
          ),
  ...
  Widget _farmStep(BuildContext context) => Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          _StepTitle(icon: Icons.business_rounded, title: tr(context, 'Farm & operation', 'المزرعة والتشغيل', 'Boerderij & bedrijf'), subtitle: tr(context, 'These details become your editable farm profile.', 'هذه البيانات ستظهر في ملف المزرعة ويمكن تعديلها.', 'Deze gegevens komen in je bewerkbare boerderijprofiel.')),
          const SizedBox(height: 18),
          _Field(controller: company, label: tr(context, 'Company name', 'اسم الشركة', 'Bedrijfsnaam'), icon: Icons.apartment_rounded),
          const SizedBox(height: 12),
          _Field(controller: farmName, label: tr(context, 'Farm / site name', 'اسم المزرعة / الموقع', 'Boerderij / locatie'), icon: Icons.home_work_outlined),
          const SizedBox(height: 12),
          Row(children: [
            Expanded(child: _Field(controller: country, label: tr(context, 'Country', 'الدولة', 'Land'), icon: Icons.public_rounded)),
            const SizedBox(width: 10),
            Expanded(child: _Field(controller: region, label: tr(context, 'Region', 'المنطقة', 'Regio'), icon: Icons.location_on_outlined)),
          ]),
          const SizedBox(height: 12),
          Row(children: [
            Expanded(child: _Field(controller: workers, label: tr(context, 'Workers', 'العمال', 'Medewerkers'), icon: Icons.groups_2_outlined, keyboard: TextInputType.number)),
            const SizedBox(width: 10),
            Expanded(child: _Field(controller: vets, label: tr(context, 'Veterinarians', 'الأطباء البيطريون', 'Dierenartsen'), icon: Icons.medical_services_outlined, keyboard: TextInputType.number)),
          ]),
          const SizedBox(height: 12),
          Row(children: [
            Expanded(child: _Field(controller: barns, label: tr(context, 'Barns', 'العنابر', 'Stallen'), icon: Icons.warehouse_outlined, keyboard: TextInputType.number)),
            const SizedBox(width: 10),
            Expanded(child: _Field(controller: area, label: tr(context, 'Indoor m²', 'المساحة م²', 'Binnen m²'), icon: Icons.square_foot_rounded, keyboard: const TextInputType.numberWithOptions(decimal: true))),
          ]),
        ],
      );
  ...
  Widget _animalsStep(BuildContext context) => Column(
        ...
        children: [
          ...
          if (selectedGroups.contains('poultry')) _Field(controller: poultry, label: tr(context, 'Poultry count', 'عدد الدواجن', 'Aantal pluimvee'), icon: Icons.tag_rounded, keyboard: TextInputType.number),
          if (selectedGroups.contains('poultry')) const SizedBox(height: 10),
          if (selectedGroups.contains('dogs')) _Field(controller: dogs, label: tr(context, 'Dog count', 'عدد الكلاب', 'Aantal honden'), icon: Icons.tag_rounded, keyboard: TextInputType.number),
          const SizedBox(height: 12),
          _Field(controller: breeds, label: tr(context, 'Breeds / strains', 'السلالات', 'Rassen / lijnen'), icon: Icons.category_outlined),
          const SizedBox(height: 12),
          _Field(controller: age, label: tr(context, 'Age / production cycle', 'العمر / دورة الإنتاج', 'Leeftijd / productiecyclus'), icon: Icons.calendar_month_outlined),
          const SizedBox(height: 12),
          _Field(controller: purpose, label: tr(context, 'Production purpose', 'غرض التربية', 'Productiedoel'), icon: Icons.flag_outlined),
          const SizedBox(height: 12),
          _Field(controller: ventilation, label: tr(context, 'Ventilation / housing', 'التهوية / الإيواء', 'Ventilatie / huisvesting'), icon: Icons.air_rounded),
          const SizedBox(height: 12),
          _Field(controller: vaccines, label: tr(context, 'Vaccination program', 'برنامج التحصينات', 'Vaccinatieprogramma'), icon: Icons.vaccines_outlined, lines: 3),
          const SizedBox(height: 12),
          _Field(controller: history, label: tr(context, 'Disease / mortality history', 'تاريخ الأمراض / النفوق', 'Ziekte- / sterftegeschiedenis'), icon: Icons.history_edu_outlined, lines: 3),
        ],
      );

  Widget _planStep(BuildContext context) {
    final groupsText = [
      if (selectedGroups.contains('livestock')) tr(context, 'Livestock', 'مواشي', 'Vee'),
      if (selectedGroups.contains('poultry')) tr(context, 'Poultry', 'دواجن', 'Pluimvee'),
      if (selectedGroups.contains('dogs')) tr(context, 'Dogs', 'كلاب', 'Honden'),
    ].join(' • ');
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        _StepTitle(icon: Icons.workspace_premium_outlined, title: tr(context, 'Choose your plan', 'اختر الخطة', 'Kies je plan'), subtitle: groupsText),
        const SizedBox(height: 18),
        _PlanCard(
          selected: plan == 'software',
          icon: Icons.health_and_safety_outlined,
          title: tr(context, 'Software only', 'البرنامج فقط', 'Alleen software'),
          lines: [
            tr(context, 'AI image assessment and health history', 'فحص الصور بالذكاء الاصطناعي والسجل الصحي', 'AI-beeldbeoordeling en gezondheidshistorie'),
            tr(context, 'Alerts from AI assessments', 'إنذارات ناتجة عن فحوصات AI', 'Meldingen uit AI-beoordelingen'),
            tr(context, 'Sensors and continuous monitoring are locked', 'الحساسات والمراقبة المستمرة غير متاحة', 'Sensoren en continue monitoring zijn vergrendeld'),
          ],
        ...
        _PlanCard(
          ...
          title: tr(context, 'Software + smart monitoring', 'البرنامج + المراقبة الذكية', 'Software + slimme monitoring'),
          lines: [
            tr(context, 'Everything in Software only', 'كل مزايا البرنامج فقط', 'Alles uit Alleen software'),
            tr(context, 'Real sensor devices and live readings when hardware is connected', 'أجهزة حساسات حقيقية وقراءات مباشرة عند توصيل الهاردوير', 'Echte sensoren en live metingen zodra hardware is gekoppeld'),
            tr(context, 'No demo sensor readings', 'بدون أي قراءات حساسات وهمية', 'Geen demo-sensormetingen'),
          ],
        ...
        SegmentedButton<String>(
          segments: [
            ButtonSegment(value: 'monthly', icon: const Icon(Icons.calendar_view_month_rounded, size: 27), label: Text(tr(context, 'Monthly', 'شهري', 'Maandelijks'))),
            ButtonSegment(value: 'annual', icon: const Icon(Icons.event_repeat_rounded, size: 27), label: Text(tr(context, 'Annual', 'سنوي', 'Jaarlijks'))),
          ],
          selected: {cycle},
        ...
        _Notice(
          ...
          title: tr(context, 'Billing status', 'حالة الدفع', 'Factureringsstatus'),
          text: tr(context, 'Plan selection is saved and controls features. Paid billing is not activated yet, so Vet AI will not pretend a payment was collected.', 'اختيار الخطة يتم حفظه ويتحكم في المزايا. الدفع المدفوع غير مفعّل بعد، لذلك Vet AI لن يدّعي أنه تم تحصيل أي مبلغ.', 'De plankiezer wordt opgeslagen en bepaalt functies. Betaalde facturering is nog niet actief; Vet AI doet dus niet alsof er betaald is.'),
        ),
      ],
    );
  }
...
class _V5DashboardState extends State<V5Dashboard> {
  ...
  Future<void> openAccount() async {
    await Navigator.of(context).push(MaterialPageRoute(builder: (_) => V5AccountHub(farm: farm)));
    await refreshFarm();
  }
...
class V5Home extends StatelessWidget {
        ...
        Card(
          child: Padding(
            ...
            child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              Row(children: [const Icon(Icons.monitor_heart_outlined, color: VetColors.primary, size: 32), const SizedBox(width: 12), Expanded(child: Text(tr(context, 'Health monitoring overview', 'نظرة عامة على المراقبة الصحية', 'Overzicht gezondheidsmonitoring'), style: const TextStyle(fontSize: 18, fontWeight: FontWeight.w900)))]),
              const SizedBox(height: 14),
              _metric(Icons.warehouse_outlined, tr(context, 'Barns', 'العنابر', 'Stallen'), farm['barn_count']),
              _metric(Icons.groups_2_outlined, tr(context, 'Workers', 'العمال', 'Medewerkers'), farm['worker_count']),
              _metric(Icons.medical_services_outlined, tr(context, 'Veterinarians', 'الأطباء البيطريون', 'Dierenartsen'), farm['veterinarian_count']),
              _metric(Icons.workspace_premium_outlined, tr(context, 'Plan', 'الخطة', 'Plan'), farm['subscription_tier'] == 'smart_monitoring' ? tr(context, 'Smart monitoring', 'مراقبة ذكية', 'Slimme monitoring') : tr(context, 'Software only', 'البرنامج فقط', 'Alleen software')),
            ]),
          ),
        ),
      ],
  ...
  Widget _metric(IconData icon, String label, dynamic value) => Padding(
        ...
        child: Row(children: [Icon(icon, color: VetColors.muted, size: 27), const SizedBox(width: 12), Text('$label: ', style: const TextStyle(color: VetColors.muted)), Expanded(child: Text('$value', style: const TextStyle(fontWeight: FontWeight.w800)))]),
      );
}
...
class _V5ScanPanelState extends State<V5ScanPanel> {
  ...
  List<String> get groups => [
        if (((widget.farm['livestock_count'] as num?)?.toInt() ?? 0) > 0) 'livestock',
  ...
  Future<void> pick(ImageSource source) async {
    ...
    final chosen = await picker.pickImage(source: source, imageQuality: 84, maxWidth: 1600);
    ...
    try {
      ...
      if (mounted) {
        setState(() => result = {
          ...
          'message': tr(context, 'The protected analysis service could not be reached.', 'تعذر الوصول إلى خدمة التحليل المحمية.', 'De beveiligde analyseservice kon niet worden bereikt.'),
        });
      }
    } finally {
      if (mounted) setState(() => busy = false);
  ...
  Widget build(BuildContext context) {
    ...
    return ListView(
      ...
      children: [
        ...
        const SizedBox(height: 16),
        if (groups.length > 1)
          Wrap(spacing: 8, runSpacing: 8, children: groups.map((g) => ChoiceChip(avatar: SizedBox(width: 56, height: 42, child: SvgPicture.asset(asset(g), fit: BoxFit.contain)), label: Text(label(g)), selected: group == g, onSelected: busy ? null : (_) => setState(() => group = g))).toList()),
        if (groups.length == 1)
        ...
        Container(
          height: 285,
          decoration: BoxDecoration(color: VetColors.surface, borderRadius: BorderRadius.circular(22), border: Border.all(color: VetColors.border)),
          child: bytes == null
              ? Center(child: Icon(Icons.add_a_photo_outlined, size: 78, color: VetColors.muted.withValues(alpha: .8)))
              : ClipRRect(borderRadius: BorderRadius.circular(22), child: Image.memory(bytes!, fit: BoxFit.cover, width: double.infinity)),
        ),
        const SizedBox(height: 12),
        Row(children: [
          Expanded(child: OutlinedButton.icon(onPressed: busy ? null : () => pick(ImageSource.camera), icon: const Icon(Icons.photo_camera_outlined, size: 29), label: Text(tr(context, 'Camera', 'الكاميرا', 'Camera')))),
          const SizedBox(width: 10),
        ...
        ElevatedButton.icon(
          ...
          icon: busy ? const SizedBox.square(dimension: 22, child: CircularProgressIndicator(strokeWidth: 2)) : const Icon(Icons.health_and_safety_outlined, size: 30),
          label: Text(busy ? tr(context, 'Analyzing safely…', 'جاري التحليل بأمان…', 'Veilig analyseren…') : tr(context, 'Analyze case', 'تحليل الحالة', 'Casus analyseren')),
        ),
        if (result != null) ...[
          const SizedBox(height: 16),
          if (complete)
  ...
  String _errorMessage(String? code) {
    switch (code) {
      case 'AI_PROVIDER_NOT_CONFIGURED':
        return tr(context, 'The protected AI provider key is not available to the Edge Function.', 'مفتاح مزود الذكاء الاصطناعي غير متاح لخدمة Edge Function.', 'De beveiligde AI-providersleutel is niet beschikbaar voor de Edge Function.');
      ...
      case 'AI_PROVIDER_AUTH_ERROR':
        return tr(context, 'The AI provider rejected the server credential. The server key needs attention.', 'مزود الذكاء الاصطناعي رفض بيانات اعتماد السيرفر. يجب مراجعة المفتاح.', 'De AI-provider heeft de serverreferentie geweigerd. De serversleutel moet worden gecontroleerd.');
      case 'SPECIES_GROUP_MISMATCH':
        return tr(context, 'The image does not appear compatible with the selected animal group. No disease guess was produced.', 'الصورة لا تبدو متوافقة مع نوع الحيوان المختار، لذلك لم يتم تخمين مرض.', 'Het beeld lijkt niet overeen te komen met de gekozen diergroep. Er is geen ziekte gegokt.');
      default:
        return tr(context, 'The case could not be analyzed. No diagnosis was invented.', 'تعذر تحليل الحالة ولم يتم اختراع تشخيص.', 'De casus kon niet worden geanalyseerd. Er is geen diagnose verzonnen.');
    }
  }
}
...
class V5HistoryPanel extends StatelessWidget {
  ...
  Widget build(BuildContext context) => FutureBuilder<List<Map<String, dynamic>>>(
        ...
        builder: (context, s) => ListView(
          padding: const EdgeInsets.all(20),
          children: [
            _StepTitle(icon: Icons.manage_history_rounded, title: tr(context, 'Assessment history', 'سجل الفحوصات', 'Beoordelingshistorie'), subtitle: tr(context, 'Stored cases and their real processing status.', 'الحالات المحفوظة وحالة معالجتها الحقيقية.', 'Opgeslagen casussen en hun echte verwerkingsstatus.')),
            const SizedBox(height: 18),
...
class V5AccountHub extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(title: Row(children: [const _MiniBrand(), const SizedBox(width: 12), Text(tr(context, 'Account', 'الحساب', 'Account'))])),
...
class _V5ProfileScreenState extends State<V5ProfileScreen> {
  ...
  void initState() {
    ...
    final f = widget.farm;
    c = {
      'full_name': TextEditingController(), 'phone': TextEditingController(), 'job_title': TextEditingController(),
      'company_name': TextEditingController(text: '${f['company_name'] ?? ''}'), 'farm_name': TextEditingController(text: '${f['farm_name'] ?? ''}'),
      'country': TextEditingController(text: '${f['country'] ?? ''}'), 'region': TextEditingController(text: '${f['region'] ?? ''}'),
      'worker_count': TextEditingController(text: '${f['worker_count'] ?? 0}'), 'veterinarian_count': TextEditingController(text: '${f['veterinarian_count'] ?? 0}'),
      'barn_count': TextEditingController(text: '${f['barn_count'] ?? 1}'), 'area': TextEditingController(text: '${f['total_indoor_area_m2'] ?? 0}'),
      'livestock_count': TextEditingController(text: '${f['livestock_count'] ?? 0}'), 'poultry_count': TextEditingController(text: '${f['poultry_count'] ?? 0}'), 'dog_count': TextEditingController(text: '${f['dog_count'] ?? 0}'),
      'breeds': TextEditingController(text: '${f['breeds'] ?? ''}'), 'age_range': TextEditingController(text: '${f['age_range'] ?? ''}'), 'production_purpose': TextEditingController(text: '${f['production_purpose'] ?? ''}'),
      'ventilation_system': TextEditingController(text: '${f['ventilation_system'] ?? ''}'), 'vaccination_notes': TextEditingController(text: '${f['vaccination_notes'] ?? ''}'), 'disease_history': TextEditingController(text: '${f['disease_history'] ?? ''}'),
    };
    load();
  }
  ...
  int _i(String key, [int fallback = 0]) => int.tryParse(c[key]!.text.trim()) ?? fallback;
  double _d(String key) => double.tryParse(c[key]!.text.trim().replaceAll(',', '.')) ?? 0;

  Future<void> save() async {
    setState(() => saving = true);
    try {
      await VetBackend.instance.updateProfile(fullName: c['full_name']!.text, phone: c['phone']!.text, jobTitle: c['job_title']!.text, preferredLanguage: Localizations.localeOf(context).languageCode);
      await VetBackend.instance.updateFarm(widget.farm['id'] as String,
        companyName: c['company_name']!.text, farmName: c['farm_name']!.text, country: c['country']!.text, region: c['region']!.text,
        workerCount: _i('worker_count'), veterinarianCount: _i('veterinarian_count'), barnCount: _i('barn_count', 1), totalIndoorAreaM2: _d('area'),
        livestockCount: _i('livestock_count'), poultryCount: _i('poultry_count'), dogCount: _i('dog_count'), breeds: c['breeds']!.text, ageRange: c['age_range']!.text,
        productionPurpose: c['production_purpose']!.text, ventilationSystem: c['ventilation_system']!.text, vaccinationNotes: c['vaccination_notes']!.text, diseaseHistory: c['disease_history']!.text);
      if (mounted) ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text(tr(context, 'Profile saved.', 'تم حفظ الملف.', 'Profiel opgeslagen.'))));
    } catch (_) {
      if (mounted) ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text(tr(context, 'Could not save the profile.', 'تعذر حفظ الملف.', 'Profiel kon niet worden opgeslagen.')), backgroundColor: Theme.of(context).colorScheme.error));
    } finally {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(title: Text(tr(context, 'Profile & farm', 'الملف والمزرعة', 'Profiel & boerderij'))),
      body: ListView(padding: const EdgeInsets.all(20), children: [
        _StepTitle(icon: Icons.account_circle_outlined, title: tr(context, 'Personal details', 'البيانات الشخصية', 'Persoonlijke gegevens'), subtitle: VetBackend.instance.currentUser?.email ?? ''),
        const SizedBox(height: 16),
        _Field(controller: c['full_name']!, label: tr(context, 'Full name', 'الاسم الكامل', 'Volledige naam'), icon: Icons.badge_outlined), const SizedBox(height: 10),
        _Field(controller: c['phone']!, label: tr(context, 'Phone', 'الهاتف', 'Telefoon'), icon: Icons.phone_outlined), const SizedBox(height: 10),
        _Field(controller: c['job_title']!, label: tr(context, 'Role / title', 'الصفة / الوظيفة', 'Rol / functie'), icon: Icons.work_outline_rounded), const SizedBox(height: 22),
        _StepTitle(icon: Icons.home_work_outlined, title: tr(context, 'Farm details', 'بيانات المزرعة', 'Boerderijgegevens'), subtitle: tr(context, 'All onboarding data is editable here.', 'كل بيانات التسجيل قابلة للتعديل هنا.', 'Alle onboardinggegevens zijn hier bewerkbaar.')),
        const SizedBox(height: 16),
        for (final spec in <(String,String,IconData)>[
          ('company_name',tr(context,'Company','الشركة','Bedrijf'),Icons.apartment_rounded), ('farm_name',tr(context,'Farm name','اسم المزرعة','Boerderijnaam'),Icons.home_work_outlined),
          ('country',tr(context,'Country','الدولة','Land'),Icons.public_rounded), ('region',tr(context,'Region','المنطقة','Regio'),Icons.location_on_outlined),
          ('worker_count',tr(context,'Workers','العمال','Medewerkers'),Icons.groups_2_outlined), ('veterinarian_count',tr(context,'Veterinarians','الأطباء البيطريون','Dierenartsen'),Icons.medical_services_outlined),
          ('barn_count',tr(context,'Barns','العنابر','Stallen'),Icons.warehouse_outlined), ('area',tr(context,'Indoor area m²','المساحة الداخلية م²','Binnenoppervlak m²'),Icons.square_foot_rounded),
          ('livestock_count',tr(context,'Livestock','المواشي','Vee'),Icons.pets_outlined), ('poultry_count',tr(context,'Poultry','الدواجن','Pluimvee'),Icons.egg_alt_outlined), ('dog_count',tr(context,'Dogs','الكلاب','Honden'),Icons.pets_rounded),
          ('breeds',tr(context,'Breeds / strains','السلالات','Rassen / lijnen'),Icons.category_outlined), ('age_range',tr(context,'Age / production cycle','العمر / دورة الإنتاج','Leeftijd / productiecyclus'),Icons.calendar_month_outlined),
          ('production_purpose',tr(context,'Production purpose','غرض التربية','Productiedoel'),Icons.flag_outlined), ('ventilation_system',tr(context,'Ventilation / housing','التهوية / الإيواء','Ventilatie / huisvesting'),Icons.air_rounded),
          ('vaccination_notes',tr(context,'Vaccination program','برنامج التحصينات','Vaccinatieprogramma'),Icons.vaccines_outlined), ('disease_history',tr(context,'Disease / mortality history','تاريخ الأمراض / النفوق','Ziekte- / sterftegeschiedenis'),Icons.history_edu_outlined),
        ]) ...[
          _Field(controller: c[spec.$1]!, label: spec.$2, icon: spec.$3, lines: spec.$1 == 'vaccination_notes' || spec.$1 == 'disease_history' ? 3 : 1, keyboard: ['worker_count','veterinarian_count','barn_count','livestock_count','poultry_count','dog_count','area'].contains(spec.$1) ? TextInputType.number : null),
          const SizedBox(height: 10),
        ],
        const SizedBox(height: 10),
        ElevatedButton.icon(onPressed: saving ? null : save, icon: saving ? const SizedBox.square(dimension: 20, child: CircularProgressIndicator(strokeWidth: 2)) : const Icon(Icons.save_outlined, size: 29), label: Text(tr(context, 'Save changes', 'حفظ التعديلات', 'Wijzigingen opslaan'))),
      ]),
    );
  }
}
...
class _V5SubscriptionScreenState extends State<V5SubscriptionScreen> {
  ...
  bool saving = false;
  @override
  void initState() { super.initState(); tier = widget.farm['subscription_tier']?.toString() ?? 'software'; cycle = widget.farm['billing_cycle']?.toString() ?? 'monthly'; }

  Future<void> save() async {
    setState(() => saving = true);
    try { await VetBackend.instance.updateSubscription(widget.farm['id'] as String, subscriptionTier: tier, billingCycle: cycle); if (mounted) Navigator.pop(context); }
    finally { if (mounted) setState(() => saving = false); }
  }

  @override
  Widget build(BuildContext context) {
    final groups = [if (((widget.farm['livestock_count'] as num?)?.toInt() ?? 0)>0) tr(context,'Livestock','مواشي','Vee'), if (((widget.farm['poultry_count'] as num?)?.toInt() ?? 0)>0) tr(context,'Poultry','دواجن','Pluimvee'), if (((widget.farm['dog_count'] as num?)?.toInt() ?? 0)>0) tr(context,'Dogs','كلاب','Honden')].join(' • ');
    return Scaffold(appBar: AppBar(title: Text(tr(context,'Subscription','الاشتراك','Abonnement'))), body: ListView(padding: const EdgeInsets.all(20), children: [
      _StepTitle(icon: Icons.workspace_premium_outlined, title: tr(context,'Plan for $groups','الخطة لـ $groups','Plan voor $groups'), subtitle: tr(context,'Feature access is enforced immediately. Billing collection is still not active.','صلاحيات المزايا تتطبق فورًا. تحصيل الدفع غير مفعّل بعد.','Functietoegang wordt direct toegepast. Betalingen worden nog niet geïnd.')),
      const SizedBox(height: 18),
      _PlanCard(selected: tier=='software', icon: Icons.health_and_safety_outlined, title: tr(context,'Software only','البرنامج فقط','Alleen software'), lines:[tr(context,'AI Scan + history + alerts','فحص AI + السجل + الإنذارات','AI-scan + historie + meldingen'),tr(context,'Sensors locked','الحساسات مقفلة','Sensoren vergrendeld')], onTap:()=>setState(()=>tier='software')),
      const SizedBox(height: 12),
      _PlanCard(selected: tier=='smart_monitoring', icon: Icons.sensors_rounded, title: tr(context,'Software + smart monitoring','البرنامج + المراقبة الذكية','Software + slimme monitoring'), lines:[tr(context,'Software features + real connected sensors','مزايا البرنامج + الحساسات الحقيقية المتصلة','Softwarefuncties + echte gekoppelde sensoren'),tr(context,'No simulated telemetry','بدون بيانات حساسات محاكاة','Geen gesimuleerde telemetrie')], onTap:()=>setState(()=>tier='smart_monitoring')),
      const SizedBox(height: 16),
      SegmentedButton<String>(segments:[ButtonSegment(value:'monthly',icon:const Icon(Icons.calendar_view_month_rounded,size:27),label:Text(tr(context,'Monthly','شهري','Maandelijks'))),ButtonSegment(value:'annual',icon:const Icon(Icons.event_repeat_rounded,size:27),label:Text(tr(context,'Annual','سنوي','Jaarlijks')))], selected:{cycle}, onSelectionChanged:(v)=>setState(()=>cycle=v.first)),
      const SizedBox(height: 18),
      _Notice(icon:Icons.info_outline_rounded,title:tr(context,'Payment integration','تكامل الدفع','Betalingsintegratie'),text:tr(context,'This page currently saves the selected entitlement. A payment provider still has to be integrated before charging customers.','هذه الصفحة تحفظ صلاحية الخطة المختارة حاليًا. ما زال يجب ربط مزود دفع قبل تحصيل أموال من العملاء.','Deze pagina slaat nu de gekozen rechten op. Er moet nog een betaalprovider worden geïntegreerd voordat klanten worden belast.')),
      const SizedBox(height:18),
      ElevatedButton.icon(onPressed:saving?null:save,icon:saving?const SizedBox.square(dimension:20,child:CircularProgressIndicator(strokeWidth:2)):const Icon(Icons.check_circle_outline_rounded,size:29),label:Text(tr(context,'Save plan','حفظ الخطة','Plan opslaan'))),
    ]));
  }
}
...
class _V5SupportScreenState extends State<V5SupportScreen> {
  ...
  Future<void> load() async { final id=await VetBackend.instance.getOrCreateSupportThread(widget.farmId); if(mounted)setState(()=>threadId=id); }
  Future<void> send() async { if(threadId==null||message.text.trim().isEmpty)return; final text=message.text; message.clear(); setState(()=>sending=true); try{await VetBackend.instance.sendSupportMessage(threadId!,text);}finally{if(mounted)setState(()=>sending=false);} }
  @override void dispose(){message.dispose();super.dispose();}

  @override
  Widget build(BuildContext context){
    return Scaffold(appBar:AppBar(title:Row(children:[const Icon(Icons.support_agent_rounded,size:32,color:VetColors.primary),const SizedBox(width:10),Text(tr(context,'Vet AI Support','دعم Vet AI','Vet AI Support'))])),body:threadId==null?const Center(child:CircularProgressIndicator()):Column(children:[
      Padding(padding:const EdgeInsets.all(14),child:_Notice(icon:Icons.verified_outlined,title:tr(context,'Real support inbox','صندوق دعم حقيقي','Echte support-inbox'),text:tr(context,'Messages are stored in your account and update live. A system receipt is automatic; a human reply appears only when a support operator answers.','الرسائل محفوظة في حسابك وتتحدث مباشرة. تأكيد الاستلام آلي، والرد البشري يظهر فقط عندما يرد موظف دعم.','Berichten worden in je account opgeslagen en live bijgewerkt. De ontvangstbevestiging is automatisch; een menselijk antwoord verschijnt alleen wanneer support reageert.'))),
      Expanded(child:StreamBuilder<List<Map<String,dynamic>>>(stream:VetBackend.instance.supportMessagesStream(threadId!),builder:(context,s){final rows=s.data??[];if(!s.hasData)return const Center(child:CircularProgressIndicator());return ListView.builder(padding:const EdgeInsets.symmetric(horizontal:14,vertical:8),itemCount:rows.length,itemBuilder:(context,i){final m=rows[i];final mine=m['sender_role']=='user';return Align(alignment:mine?AlignmentDirectional.centerEnd:AlignmentDirectional.centerStart,child:Container(margin:const EdgeInsets.only(bottom:8),padding:const EdgeInsets.symmetric(horizontal:14,vertical:11),constraints:const BoxConstraints(maxWidth:310),decoration:BoxDecoration(color:mine?VetColors.surface3:VetColors.surface2,borderRadius:BorderRadius.circular(16)),child:Column(crossAxisAlignment:CrossAxisAlignment.start,children:[Text(m['message']?.toString()??'',style:const TextStyle(height:1.35)),const SizedBox(height:4),Text(m['sender_role']=='support'?tr(context,'Support','الدعم','Support'):m['sender_role']=='system'?tr(context,'System','النظام','Systeem'):tr(context,'You','أنت','Jij'),style:const TextStyle(fontSize:11,color:VetColors.muted))])));});})),
      SafeArea(top:false,child:Padding(padding:const EdgeInsets.fromLTRB(12,8,12,12),child:Row(children:[Expanded(child:TextField(controller:message,maxLines:3,minLines:1,onSubmitted:(_)=>send(),decoration:InputDecoration(hintText:tr(context,'Write a message…','اكتب رسالة…','Schrijf een bericht…'),prefixIcon:const Icon(Icons.chat_bubble_outline_rounded,size:27)))),const SizedBox(width:8),IconButton.filled(onPressed:sending?null:send,icon:const Icon(Icons.send_rounded,size:29))]))),
    ]));
  }
}
...
class V5AboutScreen extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context)=>Scaffold(appBar:AppBar(title:Text(tr(context,'About Vet AI','عن Vet AI','Over Vet AI'))),body:ListView(padding:const EdgeInsets.all(20),children:[
    const Center(child:_BrandLockup(markWidth:150)),const SizedBox(height:20),
    _Notice(icon:Icons.favorite_outline_rounded,title:tr(context,'Purpose','الهدف','Doel'),text:tr(context,'Vet AI is being built to detect health changes earlier, organize evidence and help farmers and veterinarians act faster.','يتم بناء Vet AI لاكتشاف التغيرات الصحية مبكرًا وتنظيم الأدلة ومساعدة المربين والأطباء البيطريين على التصرف أسرع.','Vet AI wordt gebouwd om gezondheidsveranderingen eerder te signaleren, bewijs te structureren en boeren en dierenartsen sneller te laten handelen.')),
    const SizedBox(height:12),
    _Notice(icon:Icons.health_and_safety_outlined,title:tr(context,'Safety boundary','حدود الأمان','Veiligheidsgrens'),text:tr(context,'One image is never treated as a definitive diagnosis. High-consequence disease patterns trigger conservative triage, biosecurity advice and veterinary/laboratory confirmation.','لا يتم اعتبار صورة واحدة تشخيصًا نهائيًا. أنماط الأمراض عالية الخطورة تؤدي إلى فرز حذر وإرشادات أمان حيوي وتأكيد بيطري/معملي.','Eén beeld geldt nooit als definitieve diagnose. Patronen van ernstige ziekten leiden tot conservatieve triage, bioveiligheidsadvies en veterinaire/laboratoriumbevestiging.')),
    const SizedBox(height:12),
    _Notice(icon:Icons.menu_book_outlined,title:tr(context,'Knowledge base','قاعدة المعرفة','Kennisbank'),text:tr(context,'Production knowledge is source-backed and versioned. Conditions are added only after source review; the system will not claim to contain every disease in the world until that is genuinely true.','معرفة الإنتاج مرتبطة بمصادر ومُصدّرة بإصدارات. لا تتم إضافة الحالات إلا بعد مراجعة المصدر، ولن يدّعي النظام احتواء كل أمراض العالم قبل أن يكون ذلك حقيقيًا.','Productiekennis is brononderbouwd en geversioneerd. Aandoeningen worden pas na broncontrole toegevoegd; het systeem claimt niet alle ziekten ter wereld te bevatten voordat dat werkelijk zo is.')),
  ]));
}

class _Field extends StatelessWidget {
  const _Field({required this.controller,required this.label,required this.icon,this.keyboard,this.lines=1,this.obscure=false,this.onSubmitted});
  final TextEditingController controller; final String label; final IconData icon; final TextInputType? keyboard; final int lines; final bool obscure; final ValueChanged<String>? onSubmitted;
  @override Widget build(BuildContext context)=>TextField(controller:controller,keyboardType:keyboard,maxLines:obscure?1:lines,obscureText:obscure,onSubmitted:onSubmitted,decoration:InputDecoration(labelText:label,prefixIcon:Icon(icon,size:28)));
}

class _StepTitle extends StatelessWidget {
  const _StepTitle({required this.icon,required this.title,required this.subtitle}); final IconData icon; final String title; final String subtitle;
  @override Widget build(BuildContext context)=>Row(crossAxisAlignment:CrossAxisAlignment.start,children:[Container(width:48,height:48,decoration:BoxDecoration(color:VetColors.surface3,borderRadius:BorderRadius.circular(14)),child:Icon(icon,size:31,color:VetColors.primary)),const SizedBox(width:13),Expanded(child:Column(crossAxisAlignment:CrossAxisAlignment.start,children:[Text(title,style:const TextStyle(fontSize:23,fontWeight:FontWeight.w900)),const SizedBox(height:4),Text(subtitle,style:const TextStyle(color:VetColors.muted,height:1.4))]))]);
}

class _Notice extends StatelessWidget {
  const _Notice({required this.icon,required this.title,required this.text,this.danger=false}); final IconData icon; final String title; final String text; final bool danger;
...
class _AnimalGroupBanner extends StatelessWidget {
  ...
  Widget build(BuildContext context) => Container(
        ...
        child: Row(children: [
          SizedBox(width: 112, height: 84, child: SvgPicture.asset(asset, fit: BoxFit.contain)),
...
class _AnimalChoice extends StatelessWidget {
  ...
  @override Widget build(BuildContext context)=>InkWell(onTap:onTap,borderRadius:BorderRadius.circular(18),child:Container(padding:const EdgeInsets.symmetric(horizontal:16,vertical:15),decoration:BoxDecoration(color:selected?VetColors.surface3:VetColors.surface,borderRadius:BorderRadius.circular(18),border:Border.all(color:selected?VetColors.primary:VetColors.border,width:selected?1.5:1)),child:Row(children:[SizedBox(width:92,height:68,child:SvgPicture.asset(asset,fit:BoxFit.contain)),const SizedBox(width:14),Expanded(child:Text(label,style:const TextStyle(fontSize:18,fontWeight:FontWeight.w800))),Icon(selected?Icons.check_circle_rounded:Icons.radio_button_unchecked_rounded,size:29,color:selected?VetColors.primary:VetColors.muted)])));
}

class _PlanCard extends StatelessWidget {
  const _PlanCard({required this.selected,required this.icon,required this.title,required this.lines,required this.onTap}); final bool selected; final IconData icon; final String title; final List<String> lines; final VoidCallback onTap;
  @override Widget build(BuildContext context)=>InkWell(onTap:onTap,borderRadius:BorderRadius.circular(20),child:Container(padding:const EdgeInsets.all(17),decoration:BoxDecoration(color:selected?VetColors.surface3:VetColors.surface,borderRadius:BorderRadius.circular(20),border:Border.all(color:selected?VetColors.primary:VetColors.border,width:selected?1.6:1)),child:Column(crossAxisAlignment:CrossAxisAlignment.start,children:[Row(children:[Icon(icon,size:34,color:VetColors.primary),const SizedBox(width:12),Expanded(child:Text(title,style:const TextStyle(fontSize:19,fontWeight:FontWeight.w900))),Icon(selected?Icons.check_circle_rounded:Icons.radio_button_unchecked_rounded,size:29,color:selected?VetColors.primary:VetColors.muted)]),const SizedBox(height:10),for(final line in lines)Padding(padding:const EdgeInsets.only(top:6),child:Row(crossAxisAlignment:CrossAxisAlignment.start,children:[const Icon(Icons.check_rounded,size:21,color:VetColors.muted),const SizedBox(width:8),Expanded(child:Text(line,style:const TextStyle(color:VetColors.muted,height:1.35)))]))])));
}
...
class _MenuTile extends StatelessWidget {
  const _MenuTile({required this.icon,required this.title,required this.subtitle,required this.onTap}); final IconData icon; final String title; final String subtitle; final VoidCallback onTap;
  @override Widget build(BuildContext context)=>Card(margin:const EdgeInsets.only(bottom:11),child:ListTile(contentPadding:const EdgeInsets.symmetric(horizontal:16,vertical:9),leading:Icon(icon,size:34,color:VetColors.primary),title:Text(title,style:const TextStyle(fontWeight:FontWeight.w900,fontSize:17)),subtitle:Padding(padding:const EdgeInsets.only(top:3),child:Text(subtitle)),trailing:const Icon(Icons.chevron_right_rounded,size:31),onTap:onTap));
}

class _MiniBrand extends StatelessWidget {
  const _MiniBrand();
  @override Widget build(BuildContext context)=>SvgPicture.asset('assets/vet_ai_logo.svg',width:64,height:46,colorFilter:const ColorFilter.mode(VetColors.primary,BlendMode.srcIn));
}
  ...
  Widget build(BuildContext context) => Row(
        ...
        children: [
          Transform.translate(
            ...
            child: SvgPicture.asset('assets/vet_ai_logo.svg', width: 82, height: 56, colorFilter: const ColorFilter.mode(VetColors.primary, BlendMode.srcIn)),
          ),
...
class _FatalState extends StatelessWidget {
  const _FatalState({required this.message}); final String message;
  @override Widget build(BuildContext context)=>Scaffold(body:Center(child:Padding(padding:const EdgeInsets.all(24),child:Column(mainAxisSize:MainAxisSize.min,children:[const Icon(Icons.error_outline_rounded,size:58,color:VetColors.red),const SizedBox(height:14),Text(message,textAlign:TextAlign.center),const SizedBox(height:18),OutlinedButton.icon(onPressed:VetBackend.instance.signOut,icon:const Icon(Icons.logout_rounded),label:Text(tr(context,'Sign out','تسجيل الخروج','Uitloggen')))]))));
}
