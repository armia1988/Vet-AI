...
class OnvifCameraControlService {
  ...
  Future<OnvifCameraCapabilities> readCapabilities() async {
    ...
    if (ptzUri != null) {
      ...
      try {
        ...
        ptzNodeToken ??= _firstMatch(
          nodesXml,
          RegExp(r'''<(?:\w+:)?PTZNode\b[^>]*\btoken=["']([^"']+)["']''', caseSensitive: false),
        );
        ...
        presets = RegExp(r'<(?:\w+:)?MaximumNumberOfPresets\b[^>]*>([1-9]\d*)</', caseSensitive: false).hasMatch(nodesXml) ||
            (fixedHome != null && fixedHome.toLowerCase() == 'true');
      } catch (_) {
        continuous = true;
      }
    ...
    if (imagingUri != null && videoSourceToken != null && videoSourceToken.isNotEmpty) {
      try {
        final imaging = await getImagingSettingsRaw(imagingUri, videoSourceToken);
        imagingSupported = imaging.isNotEmpty;
      } catch (_) {}
    }
  ...
  Future<void> continuousMove({
    ...
    if (uri == null || !capabilities.supportsContinuousMove) {
      throw const UnsupportedError('Continuous PTZ is not reported by this camera');
    }
  ...
  Future<void> gotoPreset(OnvifCameraCapabilities capabilities, String presetToken) async {
    final uri = capabilities.ptzXAddr;
    if (uri == null) throw const UnsupportedError('PTZ is not available');
    await _soap(
      uri,
  ...
  String? _serviceXAddr(String xml, String serviceName) {
    return _firstMatch(
      xml,
  ...
  Uri? _parseOptionalUri(String? value) {
    ...
    return Uri.tryParse(_xmlUnescape(value.trim()));
  }
