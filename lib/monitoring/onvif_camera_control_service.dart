...
class OnvifCameraCapabilities {
  ...
  final String? ptzConfigurationToken;
  final Uri? ptzXAddr;
...
class OnvifCameraControlService {
  ...
  Future<OnvifCameraCapabilities> readCapabilities() async {
    ...
    final media = _serviceXAddr(capsXml, 'Media') ??
        _serviceXAddr(capsXml, 'Media2') ??
        _firstHttpXAddr(capsXml);
    if (media == null) {
      throw const FormatException('Camera did not report an ONVIF Media service');
    }
    ...
    final profileToken = _firstMatch(
      profilesXml,
      RegExp(
    ...
    if (profileToken == null || profileToken.isEmpty) {
      throw const FormatException('Camera did not report an ONVIF media profile');
    }

    final videoSourceToken = _firstMatch(
          profilesXml,
          RegExp(
            r'''<(?:\w+:)?VideoSourceConfiguration\b[^>]*\btoken=["']([^"']+)["'][^>]*>''',
            caseSensitive: false,
          ),
        ) ??
        _xmlValue(profilesXml, 'SourceToken');

    // Important: the PTZConfiguration token is not the media profile token.
    // Some cameras reject GetConfiguration when these are mixed up.
    final ptzConfigurationToken = _firstMatch(
      profilesXml,
      RegExp(
        r'''<(?:\w+:)?PTZConfiguration\b[^>]*\btoken=["']([^"']+)["']''',
        caseSensitive: false,
      ),
    );
    ...
    if (ptzUri != null) {
      if (ptzConfigurationToken != null && ptzConfigurationToken.isNotEmpty) {
        try {
          final configXml = await _soap(
            ptzUri,
            'http://www.onvif.org/ver20/ptz/wsdl/GetConfiguration',
            '<tptz:GetConfiguration xmlns:tptz="http://www.onvif.org/ver20/ptz/wsdl"><tptz:PTZConfigurationToken>${_xmlEscape(ptzConfigurationToken)}</tptz:PTZConfigurationToken></tptz:GetConfiguration>',
            tolerateFault: true,
          );
          ptzNodeToken = _xmlValue(configXml, 'NodeToken');
        } catch (_) {}

        // GetConfigurationOptions is the most direct ONVIF source for supported
        // movement spaces for this exact PTZ configuration.
        try {
          final optionsXml = await _soap(
            ptzUri,
            'http://www.onvif.org/ver20/ptz/wsdl/GetConfigurationOptions',
            '<tptz:GetConfigurationOptions xmlns:tptz="http://www.onvif.org/ver20/ptz/wsdl"><tptz:ConfigurationToken>${_xmlEscape(ptzConfigurationToken)}</tptz:ConfigurationToken></tptz:GetConfigurationOptions>',
            tolerateFault: true,
          );
          continuous = _hasAny(
            optionsXml,
      ...
      // Node data is used only as additional evidence. If probing fails, we do
      // not fabricate support for any PTZ operation.
      try {
        final nodesXml = await _soap(
          ptzUri,
          'http://www.onvif.org/ver20/ptz/wsdl/GetNodes',
          '<tptz:GetNodes xmlns:tptz="http://www.onvif.org/ver20/ptz/wsdl"/>',
          tolerateFault: true,
        );
        ptzNodeToken ??= _firstMatch(
          nodesXml,
          RegExp(
            r'''<(?:\w+:)?PTZNode\b[^>]*\btoken=["']([^"']+)["']''',
            caseSensitive: false,
          ),
        );
        ...
        final maxPresets = _xmlInt(nodesXml, 'MaximumNumberOfPresets');
        presets = maxPresets != null && maxPresets > 0;
      } catch (_) {}

      // A successful GetPresets response containing preset records is direct
      // evidence that the camera supports presets.
      try {
        ...
        presets = presets ||
            RegExp(r'<(?:\w+:)?Preset\b', caseSensitive: false)
                .hasMatch(presetsXml);
      } catch (_) {}
    }
    ...
    if (imagingUri != null &&
        videoSourceToken != null &&
        videoSourceToken.isNotEmpty) {
      try {
        imagingSupported =
            (await getImagingSettingsRaw(imagingUri, videoSourceToken))
                .isNotEmpty;
      } catch (_) {}
    }
  ...
  Future<void> continuousMove({
    required OnvifCameraCapabilities capabilities,
    double pan = 0,
    double tilt = 0,
    double zoom = 0,
  }) async {
    ...
    if (uri == null || !capabilities.supportsContinuousMove) {
      throw UnsupportedError(
        'Continuous PTZ is not reported by this camera',
      );
    }
    final body = StringBuffer()
      ..write(
        '<tptz:ContinuousMove xmlns:tptz="http://www.onvif.org/ver20/ptz/wsdl"><tptz:ProfileToken>',
      )
      ..write(_xmlEscape(capabilities.profileToken))
      ..write('</tptz:ProfileToken><tptz:Velocity>')
      ..write(
        '<tt:PanTilt xmlns:tt="http://www.onvif.org/ver10/schema" x="${pan.clamp(-1.0, 1.0)}" y="${tilt.clamp(-1.0, 1.0)}"/>',
      )
      ..write(
        '<tt:Zoom xmlns:tt="http://www.onvif.org/ver10/schema" x="${zoom.clamp(-1.0, 1.0)}"/>',
      )
  ...
  Future<void> stop(OnvifCameraCapabilities capabilities) async {
    ...
    await _soap(
      ...
      '<tptz:Stop xmlns:tptz="http://www.onvif.org/ver20/ptz/wsdl"><tptz:ProfileToken>${_xmlEscape(capabilities.profileToken)}</tptz:ProfileToken><tptz:PanTilt>true</tptz:PanTilt><tptz:Zoom>true</tptz:Zoom></tptz:Stop>',
    );
  }

  Future<List<OnvifPreset>> getPresets(
    ...
    final xml = await _soap(
      uri,
      'http://www.onvif.org/ver20/ptz/wsdl/GetPresets',
      '<tptz:GetPresets xmlns:tptz="http://www.onvif.org/ver20/ptz/wsdl"><tptz:ProfileToken>${_xmlEscape(capabilities.profileToken)}</tptz:ProfileToken></tptz:GetPresets>',
      tolerateFault: true,
    );
    ...
    for (final m in pattern.allMatches(xml)) {
      ...
      if (token.isNotEmpty) {
        out.add(
          OnvifPreset(
            token: token,
            name: name?.isNotEmpty == true ? name! : token,
          ),
        );
      }
    }
  ...
  Future<void> gotoPreset(
    ...
    await _soap(
      ...
      '<tptz:GotoPreset xmlns:tptz="http://www.onvif.org/ver20/ptz/wsdl"><tptz:ProfileToken>${_xmlEscape(capabilities.profileToken)}</tptz:ProfileToken><tptz:PresetToken>${_xmlEscape(presetToken)}</tptz:PresetToken></tptz:GotoPreset>',
    );
  }

  Future<OnvifImagingSettings?> getImagingSettings(
    OnvifCameraCapabilities capabilities,
  ) async {
  ...
  Future<String> getImagingSettingsRaw(Uri uri, String sourceToken) => _soap(
        uri,
        'http://www.onvif.org/ver20/imaging/wsdl/GetImagingSettings',
        '<timg:GetImagingSettings xmlns:timg="http://www.onvif.org/ver20/imaging/wsdl"><timg:VideoSourceToken>${_xmlEscape(sourceToken)}</timg:VideoSourceToken></timg:GetImagingSettings>',
      );
  ...
  Future<String> _soap(
    ...
    try {
      final request =
          await client.postUrl(uri).timeout(const Duration(seconds: 7));
      ...
      if (response.statusCode < 200 || response.statusCode >= 300) {
        throw HttpException('ONVIF HTTP ${response.statusCode}', uri: uri);
      }
      if (!tolerateFault &&
          RegExp(r'<(?:\w+:)?Fault\b', caseSensitive: false).hasMatch(xml)) {
        throw HttpException(
          _xmlValue(xml, 'Text') ??
  ...
  bool _hasAny(String xml, List<String> names) =>
      names.any((name) => xml.contains(name));
  ...
  String? _firstHttpXAddr(String xml) => _firstMatch(
        xml,
        RegExp(
  ...
  int? _xmlInt(String xml, String name) {
    final value = _xmlValue(xml, name);
    return value == null ? null : int.tryParse(value.trim());
  }

  double? _xmlDouble(String xml, String name) {
    final value = _xmlValue(xml, name);
