...
class OnvifCameraEvent {
  const OnvifCameraEvent({required this.topic, required this.utcTime, required this.values, this.operation});
  final String topic;
  ...
  String get summary {
    ...
    const preferred = <String>['State', 'IsMotion', 'Motion', 'Rule', 'RuleName', 'ObjectId', 'Temperature', 'MaxTemperature', 'Alarm'];
...
class OnvifEventSession {
  const OnvifEventSession({required this.pullPointUri, required this.terminationTime});
  final Uri pullPointUri;
  final DateTime? terminationTime;
}

class OnvifAnalyticsEventsService {
  const OnvifAnalyticsEventsService({required this.host, required this.httpPort, required this.username, required this.password});
  final String host;
  final int httpPort;
  ...
  Future<_ServiceAddresses> readServiceAddresses() async {
    final xml = await _soap(_deviceUri, 'http://www.onvif.org/ver10/device/wsdl/GetCapabilities', '<tds:GetCapabilities xmlns:tds="http://www.onvif.org/ver10/device/wsdl"><tds:Category>All</tds:Category></tds:GetCapabilities>');
    return _ServiceAddresses(analytics: _parseUri(_serviceXAddr(xml, 'Analytics')), events: _parseUri(_serviceXAddr(xml, 'Events')));
  }
  ...
  Future<List<OnvifAnalyticsModule>> getAnalyticsModules(String configurationToken) async {
    ...
    if (configurationToken.trim().isEmpty) throw const FormatException('Video analytics configuration token is missing');
    final xml = await _soap(uri, 'http://www.onvif.org/ver20/analytics/wsdl/GetAnalyticsModules', '<tan:GetAnalyticsModules xmlns:tan="http://www.onvif.org/ver20/analytics/wsdl"><tan:ConfigurationToken>${_xmlEscape(configurationToken)}</tan:ConfigurationToken></tan:GetAnalyticsModules>');
    final modules = <OnvifAnalyticsModule>[];
    final pattern = RegExp(r'''<(?:\w+:)?AnalyticsModule\b[^>]*\bName=["']([^"']*)["'][^>]*\bType=["']([^"']*)["'][^>]*>''', caseSensitive: false);
    for (final match in pattern.allMatches(xml)) {
      modules.add(OnvifAnalyticsModule(name: _xmlUnescape(match.group(1) ?? '').trim(), type: _xmlUnescape(match.group(2) ?? '').trim()));
    }
    return modules;
  }
  ...
  Future<List<String>> getEventTopics() async {
    ...
    final xml = await _soap(uri, 'http://www.onvif.org/ver10/events/wsdl/EventPortType/GetEventPropertiesRequest', '<tev:GetEventProperties xmlns:tev="http://www.onvif.org/ver10/events/wsdl"/>');
    final topics = <String>{};
    ...
    final descriptions = RegExp(r'''<(?:\w+:)?MessageDescription\b[^>]*\bIsProperty=["'](?:true|false)["'][^>]*>''', caseSensitive: false).allMatches(xml).length;
  ...
  Future<OnvifEventSession> createPullPointSubscription() async {
    final uri = (await readServiceAddresses()).events;
    if (uri == null) throw UnsupportedError('Camera did not report an ONVIF Events service');
    final xml = await _soap(uri, 'http://www.onvif.org/ver10/events/wsdl/EventPortType/CreatePullPointSubscriptionRequest', '<tev:CreatePullPointSubscription xmlns:tev="http://www.onvif.org/ver10/events/wsdl"><tev:InitialTerminationTime>PT10M</tev:InitialTerminationTime></tev:CreatePullPointSubscription>');
    final address = _xmlValue(xml, 'Address');
    if (address == null || address.trim().isEmpty) throw const FormatException('PullPoint subscription address missing');
    final termination = _xmlValue(xml, 'TerminationTime');
    return OnvifEventSession(pullPointUri: Uri.parse(_xmlUnescape(address.trim())), terminationTime: termination == null ? null : DateTime.tryParse(termination.trim()));
  }

  Future<List<OnvifCameraEvent>> pullMessages(OnvifEventSession session, {Duration timeout = const Duration(seconds: 5), int messageLimit = 20}) async {
    final seconds = timeout.inSeconds.clamp(1, 30);
    final limit = messageLimit.clamp(1, 100);
    final xml = await _soap(session.pullPointUri, 'http://www.onvif.org/ver10/events/wsdl/PullPointSubscription/PullMessagesRequest', '<tev:PullMessages xmlns:tev="http://www.onvif.org/ver10/events/wsdl"><tev:Timeout>PT${seconds}S</tev:Timeout><tev:MessageLimit>$limit</tev:MessageLimit></tev:PullMessages>', timeout: Duration(seconds: seconds + 6));
    return _parseNotificationMessages(xml);
  }

  Future<void> unsubscribe(OnvifEventSession session) async {
    try {
      await _soap(session.pullPointUri, 'http://docs.oasis-open.org/wsn/bw-2/SubscriptionManager/UnsubscribeRequest', '<wsnt:Unsubscribe xmlns:wsnt="http://docs.oasis-open.org/wsn/b-2"/>');
    } catch (_) {}
  }

  List<OnvifCameraEvent> _parseNotificationMessages(String xml) {
    final out = <OnvifCameraEvent>[];
    final pattern = RegExp(r'<(?:\w+:)?NotificationMessage\b[^>]*>(.*?)</(?:\w+:)?NotificationMessage>', dotAll: true, caseSensitive: false);
    for (final match in pattern.allMatches(xml)) {
      ...
      final simple = RegExp(r'''<(?:\w+:)?SimpleItem\b[^>]*\bName=["']([^"']+)["'][^>]*\bValue=["']([^"']*)["'][^>]*/?>''', caseSensitive: false);
      for (final item in simple.allMatches(messageBody)) {
        final name = _xmlUnescape(item.group(1) ?? '').trim();
        if (name.isNotEmpty) values[name] = _xmlUnescape(item.group(2) ?? '').trim();
      }
      final utcRaw = _attribute(attrs, 'UtcTime');
      out.add(OnvifCameraEvent(topic: _xmlUnescape(topic), utcTime: utcRaw == null ? null : DateTime.tryParse(utcRaw), values: values, operation: _attribute(attrs, 'PropertyOperation')));
    }
    return out;
  }

  Future<String> _soap(Uri uri, String action, String body, {Duration timeout = const Duration(seconds: 10)}) async {
    ...
    if (second.statusCode < 200 || second.statusCode >= 300) throw HttpException('ONVIF HTTP ${second.statusCode}', uri: uri);
  ...
  void _throwOnSoapFault(String xml, Uri uri) {
    if (RegExp(r'<(?:\w+:)?Fault\b', caseSensitive: false).hasMatch(xml)) throw HttpException(_xmlValue(xml, 'Text') ?? _xmlValue(xml, 'faultstring') ?? 'ONVIF SOAP fault', uri: uri);
  }
  ...
  String _digestAuthorization(String challenge, String method, Uri uri) {
    ...
    final useAuth = (params['qop'] ?? '').split(',').map((e) => e.trim().toLowerCase()).contains('auth');
    String response;
    String suffix = '';
    if (useAuth) {
      const nc = '00000001';
