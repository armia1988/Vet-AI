...
class SupportCallBar extends StatelessWidget {
  ...
  Widget build(BuildContext context) => StreamBuilder<List<Map<String, dynamic>>>(
        ...
        builder: (context, snapshot) {
          final active = (snapshot.data ?? const <Map<String, dynamic>>[]).where((c) => c['status'] == 'ringing').toList();
          ...
          if (!incoming) {
            return Container(
              ...
              child: Row(children: [
                ...
                TextButton(onPressed: () => VetSupportCallService.instance.end(call['id'].toString()), child: Text(_cbt(context, 'End', 'إنهاء', 'Stop'))),
              ]),
            );
          }
          return Container(
            ...
            child: Row(children: [
              ...
              const SizedBox(width: 8),
              Expanded(child: Text(call['call_type'] == 'video'
                  ? _cbt(context, 'Incoming video call', 'مكالمة فيديو واردة', 'Inkomend videogesprek')
                  : _cbt(context, 'Incoming voice call', 'مكالمة صوتية واردة', 'Inkomende spraakoproep'), style: const TextStyle(fontWeight: FontWeight.w900))),
              TextButton(onPressed: () => VetSupportCallService.instance.decline(call['id'].toString()), child: Text(_cbt(context, 'Decline', 'رفض', 'Weigeren'))),
