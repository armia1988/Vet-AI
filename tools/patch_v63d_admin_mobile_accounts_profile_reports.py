...
def replace_once(text: str, old: str, new: str, label: str) -> str:
    if new in text:
        return text
    if old not in text:
        raise SystemExit(f'V63d: {label} anchor missing')
    return text.replace(old, new, 1)


# Account list: live local refresh and responsive cards.
...
# Staff dialog labels and refresh.
...
''',
    '''                const _AdminBrandHeader(),
                _AdminProfileTile(role: role ?? 'admin', onTap: _openMyAccount),
...
''',
    '''                  Padding(
                    padding: const EdgeInsets.all(12),
                    child: _AdminProfileTile(role: role ?? 'admin', onTap: _openMyAccount, compact: true),
                  ),
''',
    'desktop account tile',
)
if 'class _AdminProfileTile extends StatelessWidget' not in s:
    s = replace_once(
        s,
        'class _AdminDestination {',
        r'''class _AdminProfileTile extends StatelessWidget {
  const _AdminProfileTile({required this.role, required this.onTap, this.compact = false});
  final String role;
    ...
    return Material(
      ...
      child: InkWell(
        ...
        child: Padding(
          ...
          child: Row(children: [
            CircleAvatar(radius: compact ? 18 : 20, backgroundColor: VetColors.softGreen, child: const Icon(Icons.person_rounded, color: VetColors.green)),
            const SizedBox(width: 10),
            Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
...
# Mobile operations cards: two columns.
...
for path, markers in checks.items():
    ...
    for marker in markers:
        if marker not in text:
            raise SystemExit(f'V63d verification missing {marker} in {path}')

print('Vet AI V63d applied: responsive accounts, automatic local refresh, stable RBAC labels, editable own profile, 2-column mobile dashboard and live intelligence reports')
