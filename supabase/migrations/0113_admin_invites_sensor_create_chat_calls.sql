-- Vet AI Admin A-Z expansion: account invitations, sensor provisioning, support call metadata.
...
create table if not exists public.admin_account_invites (
  ...
  kind text not null default 'customer' check (kind in ('customer','admin','support','operations','billing','worker')),
  admin_role text,
  permissions jsonb not null default '{}'::jsonb,
  farm_id uuid references public.farms(id) on delete set null,
  member_role public.member_role,
  create_farm boolean not null default false,
  company_name text,
  farm_name text,
  country text,
  region text,
...
on public.admin_account_invites (lower(email))
...
create policy admin_account_invites_admin_read on public.admin_account_invites
for select to authenticated using (public.is_admin_account('manage_admins'));
drop policy if exists admin_account_invites_admin_manage on public.admin_account_invites;
create policy admin_account_invites_admin_manage on public.admin_account_invites
for all to authenticated using (public.is_admin_account('manage_admins')) with check (public.is_admin_account('manage_admins'));
...
create or replace function public.admin_create_account_invite(
  ...
  p_create_farm boolean default false,
...
begin
  ...
  if p_member_role is not null and trim(p_member_role) <> '' then
    begin v_member_role := trim(p_member_role)::public.member_role;
    exception when others then raise exception 'Invalid farm member role'; end;
  end if;
  if p_create_farm and trim(coalesce(p_farm_name,''))='' then raise exception 'Farm name required'; end if;

  delete from public.admin_account_invites
  where lower(email)=v_email and accepted_at is null;

  v_token := encode(gen_random_bytes(32),'hex');
  insert into public.admin_account_invites(
    email,full_name,phone,preferred_language,kind,admin_role,permissions,
    farm_id,member_role,create_farm,company_name,farm_name,country,region,
    token_hash,created_by
  ) values (
    v_email,nullif(trim(coalesce(p_full_name,'')),''),nullif(trim(coalesce(p_phone,'')),''),
    coalesce(nullif(trim(p_preferred_language),''),'en'),p_kind,p_admin_role,coalesce(p_permissions,'{}'::jsonb),
    p_farm_id,v_member_role,coalesce(p_create_farm,false),nullif(trim(coalesce(p_company_name,'')),''),
    nullif(trim(coalesce(p_farm_name,'')),''),nullif(trim(coalesce(p_country,'')),''),nullif(trim(coalesce(p_region,'')),''),
...
create or replace function public.account_invite_preview(p_token text)
returns jsonb
language plpgsql
stable
security definer
...
as $$
declare
  v public.admin_account_invites%rowtype;
...
begin
  ...
  update public.profiles set
    ...
    preferred_language=coalesce(v.preferred_language,preferred_language),
  ...
  if v.kind in ('admin','support','operations','billing') then
    ...
    values(auth.uid(),v_role,true,coalesce(v.permissions,'{}'::jsonb))
  ...
  if v.create_farm then
    ...
    values(auth.uid(),v.company_name,coalesce(v.farm_name,'Vet AI farm'),v.country,v.region)
    returning id into v_farm;
  elsif v.farm_id is not null then
    v_farm := v.farm_id;
    insert into public.farm_members(farm_id,user_id,role)
    values(v.farm_id,auth.uid(),coalesce(v.member_role,'worker'::public.member_role))
...
-- Admin sensor provisioning. Uses a server permission check rather than farm membership.
...
begin
  ...
  values(p_farm_id,trim(p_device_uid),nullif(trim(coalesce(p_display_name,'')),''),trim(p_device_type),nullif(trim(coalesce(p_section_name,'')),''),nullif(trim(coalesce(p_firmware_version,'')),''),coalesce(p_active,true),nullif(trim(coalesce(p_admin_notes,'')),''))
  returning id into v_id;
  return v_id;
end; $$;
...
-- Support-call records for voice/video session lifecycle. Media transport can use a dedicated WebRTC provider later.
create table if not exists public.support_calls (
  id uuid primary key default gen_random_uuid(),
  thread_id uuid not null references public.support_threads(id) on delete cascade,
  initiated_by uuid not null references auth.users(id) on delete cascade,
  caller_role text not null check (caller_role in ('user','support')),
  call_type text not null check (call_type in ('voice','video')),
  status text not null default 'ringing' check (status in ('ringing','accepted','declined','ended','missed')),
  room_key text not null unique default encode(gen_random_bytes(18),'hex'),
  created_at timestamptz not null default now(),
...
create policy support_calls_thread_access on public.support_calls
for all to authenticated
using (
  public.is_admin_account('support') or exists(
    select 1 from public.support_threads st where st.id=thread_id and public.is_farm_member(st.farm_id)
  )
)
with check (
  public.is_admin_account('support') or exists(
